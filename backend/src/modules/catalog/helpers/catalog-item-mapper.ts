...
import { ProductForCatalog, ProductVariantForCatalog, VariantAttributeForCatalog } from "../types/mapper.types";
...
export class CatalogItemMapper {
    static fromProduct(product: ProductForCatalog): CatalogItem[] {
        ...
        if (!product.variants || product.variants.length === 0) throw new Error("Product must have at least one variant")
